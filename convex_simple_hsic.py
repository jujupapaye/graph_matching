...
def create_objective_function(K,L,pi,c):
    def objective_function(beta):
        ...
        #return -1*np.trace(K @ beta.T @ L @ pi) + c *(constraints)
        return res
...
def create_gradient_function(K,L,pi,c):
    def gradient_function(beta):
        n = K.shape[0]
        #result =  2 *(L @ pi @ K)
        #result = -2*(beta.T @ L.T @ K + L.T @ K @ beta.T) + 2*(beta @ K.T @ K +  L.T @ L @ beta)
...
#estime une permutation p pour comparer 2 objets (convex kernelized sorting) avec K et L matrices noyaux centrés des ensembles à comparer
